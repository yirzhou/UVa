...
array robot where robot[i] is the position of the ith robot. You are also given
a 2D integer array factory where factory[j] = [positionj, limitj] indicates that
positionj is the position of the jth factory and that the jth factory can repair
...
not need to move. The limit of the first factory is 2, and it fixed 2 robots.
...
repaired at the first factory. The limit of the first factory is 1, and it fixed
1 robot. The limit of the second factory is 1, and it fixed 1 robot. The total
distance is |2 - 1| + |(-2) - (-1)| = 2. It can be shown that we cannot achieve
a better total distance than 2.
*/
const ll INFTY = 2e11 + 5;
ll memo[105][105][105];
class Solution {
  public:
    vi R;
    vvi F;
    ll dp(int r, int f, int k) {
        if (f == F.size()) return r == R.size() ? 1 : INFTY;
        if (r == R.size()) return 0;
        if (memo[r][f][k] != -1) return memo[r][f][k];
        ll ans = INFTY;

        if (k > 0) ans = abs(F[f][0] - R[r]) + dp(r + 1, f, k - 1);
        ans = min(ans, dp(r, f + 1, (f + 1 == F.size() ? 0 : F[f + 1][1])));
        return memo[r][f][k] = ans;
    }
    ...
    long long minimumTotalDistance(vector<int> &robot,
        ...
        F = factory;
        sort(ALL(R));
        sort(ALL(F), [](const vi &a, const vi &b) { return a[0] < b[0]; });
