...
int main()
{
    ...
    while (cin >> a >> b >> c) {
        double out_r = r_circum_circle(a, b, c);
        double v_area = area(a, b, c);
        double in_r = r_in_circle(a, b, c);

        double out_area = M_PI * pow(out_r, 2.0);
        double in_area = M_PI * pow(in_r, 2.0);
        cout << out_area - v_area << " " << v_area - in_area << " " << in_area << endl;
    }
}